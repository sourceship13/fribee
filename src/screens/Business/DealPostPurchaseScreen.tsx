...
const IAP_ENVIRONMENT: 'staging' | 'production' = __DEV__
  ? 'staging'
  : 'production';
...
const DealPostPurchaseScreen = ({ navigation }: any) => {
        ...
        try {
          ...
          if (response.success) {
            ...
            if (
              ...
              errorText.includes('unique_transaction') ||
              errorText.includes('violates')
            ) {
              console.log(
                '⚠️ Duplicate transaction detected - finishing anyway',
              );
            ...
            [
              {
                ...
                onPress: async () => {
                  ...
                  if (Platform.OS === 'android') {
                    console.log(
                      '🤖 Consuming Android purchase (error case)...',
                    );
                    await RNIap.consumePurchaseAndroid(purchase.purchaseToken);
                    console.log('✅ Android purchase consumed');
                  }
  ...
  const initializeIAP = async () => {
    try {
      ...
      if (products && products.length > 0) {
        ...
        console.warn(
          '⚠️ No products found! SKU may not exist or not be activated in Play Console',
        );
        Alert.alert(
  ...
  const handlePurchase = async () => {
    ...
    if (TEST_MODE_BACKEND_ONLY) {
      ...
      try {
        ...
        const mockPurchaseToken = `mock_deal_token_${Date.now()}_${Math.random()
          .toString(36)
          .substring(7)}`;
    ...
    try {
      ...
      if (
        error.message?.includes('sku not found') ||
        error.code === 'unknown'
      ) {
        Alert.alert(
          'Product Not Found',
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      {/* Footer Info */}
      <View
        style={[styles.footerInfo, { backgroundColor: colors.border + '30' }]}
      >
